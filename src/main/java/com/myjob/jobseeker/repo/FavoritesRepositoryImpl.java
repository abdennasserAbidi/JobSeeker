...
public class FavoritesRepositoryImpl implements FavoritesRepository {
    ...
    @Override
    public Page<FavoriteModel> findPaginatedFavorites(int id, int page, int size) {
        ...
        Aggregation aggregation = Aggregation.newAggregation(
            ...
            Aggregation.unwind("favorites"),
            Aggregation.replaceRoot("favorites"),
            Aggregation.skip(skip),
            Aggregation.limit(size)
        );

        // Execute the aggregation
    AggregationResults<FavoriteModel> results = mongoTemplate.aggregate(
        aggregation,
        "User", // The collection name
        FavoriteModel.class // The class type to map the results
    );

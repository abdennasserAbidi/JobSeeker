...
public class EducationRepositoryImpl implements EducationRepository {
    ...
    @Override
    public Page<Education> findPaginatedEducations(int userId, int page, int size) {
        ...
        Aggregation aggregation = Aggregation.newAggregation(
            ...
            Aggregation.unwind("education"),
            Aggregation.replaceRoot("education"),
            Aggregation.skip(skip),
            Aggregation.limit(size)
        );

        // Execute the aggregation
    AggregationResults<Education> results = mongoTemplate.aggregate(
        aggregation,
        "User", // The collection name
        Education.class // The class type to map the results
    );
